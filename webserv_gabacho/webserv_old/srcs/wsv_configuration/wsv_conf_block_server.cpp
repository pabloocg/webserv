					  ...
					  struct wsv_conf_data_s* cd)
{
	...
	servers->cgi = std::map<std::string,std::string>();
						  ...
						  struct wsv_conf_data_s* cd)
{
	...
	{
	case 3:
		if (wsv_str_cmp_3(cd->key_begin, "cgi"))
		{
			// return (wsv_conf_directive_string(cd->value_begin,
			// 								  cd->value_end,
			// 								  &conf->cgi));
			std::string extension, cgiExecutable;
			size_t n = 0;
			while(	cd->value_begin[n] != ' ' &&
					cd->value_begin[n] != '\0')
				++n;
			if(cd->value_begin[n] == '\0')
				return WSV_ERROR;
			extension = std::string(cd->value_begin, n);
			if(cd->value_begin + (n + 1) <= cd->value_end)
				cgiExecutable = std::string(cd->value_begin + (n + 1), cd->value_end);
			else
				return WSV_ERROR;
			srv->cgi[extension] = cgiExecutable;
			return WSV_OK;
		}
		break;
	case 6:
